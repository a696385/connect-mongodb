...
var MONGOSTORE = module.exports = function MongoStore(options, callback) {
  ...
  if (options.url) {
    ...
    if (urls.length > 1){
        //parse first url for authorization and db name
        url = require('url').parse(urls.splice(0,1)[0]);
        ...
        //parse another servers
        var anotherServer = urls.map(function(el){
            var srv =  require('url').parse(el);
            return new mongo.Server(srv.hostname || _defaults.host, +srv.port || _defaults.port, options);
        });
        var mongos = new mongo.Mongos(anotherServer);
        //create connection
        ...
        options.db = new mongo.Db(
          url.pathname.replace(/^\//, '')
        , new mongo.Server(url.hostname || _defaults.host, +url.port || _defaults.port, options)
        , {
            safe: options.safe,
            w: options.w,
            journal: options.journal,
